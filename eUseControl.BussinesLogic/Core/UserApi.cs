...
namespace eUseControl.BussinesLogic.Core
{
    ...
    public class UserApi
    {
        internal ULoginResp UserRegisterAction(URegisterData data)
        {
            ...
            {
                var existingUser = db.Users.FirstOrDefault(u => u.Username == data.Credential);
                if (existingUser != null)
                {
                    // Handle the case where the username already exists, maybe return an error response
                    return new ULoginResp { Status = false, ErrorMessage = "Username already exists" };
                }
